...
rm -rf "GeneralUser GS v1.511"


mkdir -p kernels kernels-hdmi
# Build and archive kernel
make clean && make -j BOARD=$BOARD # replace with your board
mv kernel*.img kernels/

# Build (HDMI console) and archive kernel
make clean && make -j BOARD=$BOARD HDMI_CONSOLE=1 # replace with your board
mv kernel*.img kernels-hdmi/
